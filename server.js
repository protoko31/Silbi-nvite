...
mongoose.connect('mongodb+srv://recep:bjk1903arda@backupbotu.p1zc8.mongodb.net/backupbotu?retryWrites=true&w=majority', {useNewUrlParser: true, useUnifiedTopology: true});// Mongo connect linki
// Recep Efsanesi 
    ...
    Database.findOne({ guildID: member.guild.id, userID: inviter.id }, (err, inviterData) => {
      if (!inviterData) {
        ...
        newInviter.save().then(x => {
          if (inviteChannel) inviteChannel.send(`${member} Hosgeldiniz Sunucumuza  
          
          \`Sizi Davet Eden Üye ${inviter.id == member.guild.id ? member.guild.name : inviter.tag} 
          
          Toplam Davet Etmis Üye ${(x.regular ? x.regular : 0)+(x.bonus ? x.bonus : 0)} Token Ve Yeni Acilen Hesapları İnvite Olarak Saymıyoruz (❌)`).catch(err => {});
              });
      } else {
        inviterData.fake++
    ...
    Database.findOne({ guildID: member.guild.id, userID: inviter.id }, (err, inviterData) => {
        if (!inviterData) {
          ...
          newInviter.save().then(x => {
            ...
            Toplam Davet Etmis Üye ${(x.regular ? x.regular : 0)+(x.bonus ? x.bonus : 0)} Güvenilir Hesap ✅)`).catch(err => {});
          });
        } else {
          inviterData.regular++;
          inviterData.save().then(x => {
